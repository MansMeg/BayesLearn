...
MH.algo<-function(logPostFunc,start.theta,no.sim,burn.in=2000,...){
  library(mvtnorm)
...
Using this likelihood and the function given i a) the (block version) Metropolis function were used and ```r sim``` simulations were done.
...
To conter this problem a new MH algorith were created where instead of a identity matrix is used for the proposal distribution, the maximum and the hessian of the liklihood is found using the ```optim``` function. The maximum is used as the starting values and the $-H^{-1}$ is used as the covariance ($\Sigma$) the proposal distribution. This is implemented in R the following way:
