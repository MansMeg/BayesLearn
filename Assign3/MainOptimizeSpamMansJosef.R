...
res.post.draw<-data.frame(mode=OptimResults$par,sd.post=sqrt(diag(-solve(OptimResults$hessian))))
rownames(res.post.draw) <- covNames # Naming the coefficient by covariates
